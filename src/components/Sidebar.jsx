              ...
              <nav key={section.section} className="flex flex-col gap-4">
                ...
                <ul className="flex flex-col gap-2">
                  {visibleItems.map((item) => {
                        ...
                        <li key={item.label} className="group flex flex-col">
                          ...
                          >
                            <div className="relative flex size-6 shrink-0">
                              <img src={item.iconBlack} className="size-6" alt="icon" />
                            </div>
                                    ...
                                    <li key={child.label} className={`group ${isChildActive ? "active" : ""}`}>
                                      ...
                                      >
                                        <div className="relative flex size-6 shrink-0">
                                          <img
                                            src={child.iconBlack}
                                            className={`size-6 absolute ${isChildActive ? "opacity-0" : "opacity-100"} transition-300`}
                                            alt="icon"
                                          />
                                          <img
                                            src={child.iconBlue}
                                            className={`size-6 absolute ${isChildActive ? "opacity-100" : "opacity-0"} transition-300`}
                                            alt="icon"
                                          />
                                        </div>
                                        <p className={`font-medium transition-300 w-full ${isChildActive ? "text-monday-blue" : ""}`}>
                                          {child.label}
                                        </p>
                                        <div className={`w-2 h-9 shrink-0 rounded-l-xl bg-monday-blue hidden ${isChildActive ? "flex" : ""} transition-300`}></div>
                                      </Link>
                                    </li>
                                  );
                                })}
                    ...
                    return (
                      <li key={item.label} className={`group ${isActive ? "active" : ""}`}>
                        <Link
                          to={item.path}
                          className="flex items-center w-full min-h-14 gap-2 rounded-2xl overflow-hidden py-[10px] pl-4 transition-300"
                        >
                          <div className="relative flex size-6 shrink-0">
                            <img
                              src={item.iconBlack}
                              className={`size-6 absolute ${isActive ? "opacity-0" : "opacity-100"} transition-300`}
                              alt="icon"
                            />
                            <img
                              src={item.iconBlue}
                              className={`size-6 absolute ${isActive ? "opacity-100" : "opacity-0"} transition-300`}
                              alt="icon"
                            />
                          </div>
                          <p className={`font-medium transition-300 w-full ${isActive ? "text-monday-blue" : ""}`}>
                            {item.label}
                          </p>
                          <div className={`w-2 h-9 shrink-0 rounded-l-xl bg-monday-blue hidden ${isActive ? "flex" : ""} transition-300`}></div>
                        </Link>
                      </li>
                    );
                  })}
